...
while True:
    ...
    lms = full_body.findLandmarkloc(color_frame, False)
    ...
    if len(lms) != 0:
        ...
        cv2.circle(color_frame, (right_shoulder_x, right_shoulder_y), 5, (0, 0, 255), -1)
        cv2.circle(color_frame, (right_elbow_x, right_elbow_y), 5, (0, 0, 255), -1)
        cv2.circle(color_frame, (right_wrist_x, right_wrist_y), 5, (0, 0, 255), -1)
        ...
        if right_wrist_x > upper_bond[0] or right_wrist_x < lower_bond[0] or right_wrist_y < lower_bond[1]:
            ...
            if right_wrist_y + 75 > right_elbow_y:
                cv2.putText(color_frame, f'No operation', operation_point, cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 2)
            else:
                if right_elbow_y < right_shoulder_y:
                    if frame_delay % 5 == 0:
                        x, y = right_wrist_x, lower_bond[1]
                        dist2 = math.dist(lower_bond, (x, y))
                        frac = round((dist2 / distance), 2)
                        frame_delay = 0
                    cv2.putText(color_frame, f'FULL,scale:{frac}', operation_point, cv2.FONT_HERSHEY_PLAIN, 3,
                                (0, 255, 0), 2)
                    ...
                    if frame_delay % 5 == 0:
                        ...
                        frac = round((dist2 / distance), 2)
                        frame_delay = 0
                    cv2.putText(color_frame, f'HALF,scale:{frac}', operation_point, cv2.FONT_HERSHEY_PLAIN, 3,
                                (0, 255, 0), 2)
